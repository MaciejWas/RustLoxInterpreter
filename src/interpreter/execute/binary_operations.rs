...
pub fn eval_err() -> ErrBuilder {
    ErrBuilder::new().with_type(LogicError)
}

pub fn cant_perform_binary_on<O: std::fmt::Debug, A: std::fmt::Debug, B: std::fmt::Debug>(op: O, a: A, b: B) -> ErrBuilder {
...
pub fn handle(op: &Token, acc: LoxValue, val: LoxValue, curr_pos: Position) -> LoxResult<LoxValue> {
    match op.as_punct()? {
        ...
        _ => Err(eval_err().with_pos(curr_pos).is_not(op, "a valid lox operation").build())
    }
}
...
fn plus(acc: LoxValue, val: LoxValue, pos: Position) -> LoxResult<LoxValue> {
    match (&acc, &val) {
        ...
        _ => Err(cant_perform_binary_on("plus", acc, val).with_pos(pos).build())
    }
}
...
fn star(acc: LoxValue, val: LoxValue, pos: Position) -> LoxResult<LoxValue> {
    match (&acc, &val) {
        (LoxValue::Integer(x), LoxValue::Integer(y)) => Ok(LoxValue::Integer(x * y)),
        (LoxValue::Boolean(x), LoxValue::Boolean(y)) => Ok(LoxValue::Boolean(*x && *y)),
        _ => Err(cant_perform_binary_on("mul", acc, val).with_pos(pos).build())
...
fn minus(acc: LoxValue, val: LoxValue, pos: Position) -> LoxResult<LoxValue> {
    match (&acc, &val) {
        (LoxValue::Integer(x), LoxValue::Integer(y)) => Ok(LoxValue::Integer(x - y)),
        (LoxValue::Boolean(x), LoxValue::Boolean(y)) => Ok(LoxValue::Boolean(*x && !*y)),
        _ => Err(cant_perform_binary_on("subtraction", acc, val).with_pos(pos).build())
    }
}
...
fn eq(acc: LoxValue, val: LoxValue, pos: Position) -> LoxResult<LoxValue> {
    match (&acc, &val) {
        ...
        _ => Err(cant_perform_binary_on("equality check", acc, val).with_pos(pos).build())
    }
}
